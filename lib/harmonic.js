...
export function orderSet(tracks, opts = {}) {
  ...
  while (remaining.length) {
    ...
    // Target energy along a build->peak(~0.7)->cooldown arc.
    const target = arc
      ? (progress < 0.7 ? 0.3 + progress : 1.0 - (progress - 0.7) * 1.5)
      : energyOf(last);
  ...
  for (let i = 0; i < order.length - 1; i++) {
    ...
    transitions.push({
      ...
      flag: s >= 0.8 ? "smooth" : s >= 0.55 ? "ok" : "risky",
      // Suggested native-crossfade length: longer for closer tempos.
