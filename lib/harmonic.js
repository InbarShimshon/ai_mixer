...
// Build a set AROUND anchor (kept) songs. The anchors are the backbone; pool songs
// are used first to BRIDGE the roughest junctions between songs (insert the song
// that best smooths the worst gap), then the rest fill in at their smoothest slots.
export function buildAround(anchors, pool) {
  let seq = anchors.length ? orderSet(anchors).order : [];
  const remaining = pool.slice();

  // Pass 1 — bridge the worst junctions while we have songs and rough gaps.
  let guard = 0;
  while (remaining.length && guard++ < 800) {
    let worstJ = -1, worst = Infinity;
    for (let j = 0; j < seq.length - 1; j++) {
      const s = transitionScore(seq[j], seq[j + 1]);
      if (s < worst) { worst = s; worstJ = j; }
    }
    if (worstJ < 0 || worst >= 0.8) break; // nothing rough left to bridge
    const a = seq[worstJ], b = seq[worstJ + 1];
    let bi = -1, best = -Infinity;
    remaining.forEach((t, k) => {
      const bridge = Math.min(transitionScore(a, t), transitionScore(t, b)); // smooth on BOTH sides
      if (bridge > best) { best = bridge; bi = k; }
    });
    if (bi < 0 || best <= worst) break; // no song improves this gap
    seq.splice(worstJ + 1, 0, remaining.splice(bi, 1)[0]);
  }

  // Pass 2 — place any leftovers at their smoothest slot.
