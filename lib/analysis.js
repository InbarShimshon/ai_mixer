...
// GetSongBPM returns key_of like "Cm", "G#m", "Db", "F" (note + optional "m").
// No trailing "m" => major.
export function toCamelot(keyOf) {
  if (!keyOf) return null;
  let s = keyOf.trim();
  let isMajor = true;
  if (/m$/.test(s) && !/^.{1,2}aj/i.test(s)) {
    isMajor = false;
    s = s.slice(0, -1);
  }
  const pc = NOTE_INDEX[s];
...
export async function lookup({ name, artist }, env) {
  ...
  try {
    ...
    return {
      ...
      camelot: toCamelot(hit.key_of),
      energy: typeof hit.danceability === "number" ? hit.danceability / 100 : null,
      source: "getsongbpm",
    };
  } catch {
